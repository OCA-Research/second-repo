...
if not os.path.exists(DATA_FILENAME):
    ...
    print("Please run 'generate_simulated_data.py' first to create the data file.")
    exit() # Exit if data file is not found

df_raw = pd.read_csv(DATA_FILENAME)
print(f"Data loaded. Initial shape: {df_raw.shape}")
print(f"Actual tampering rate (by customer in loaded data): {df_raw.drop_duplicates(subset=['customer_id'])['is_tampering_customer'].mean():.4f}")
...
print(f"Features (X_xgb) shape for XGBoost: {X_xgb.shape}")
print(f"Target (y_xgb) shape for XGBoost: {y_xgb.shape}")
...
print(f"X_scaled_sequences_dl shape for Autoencoder: {X_scaled_sequences_dl.shape}")
print(f"y_customer_labels_dl shape for Autoencoder: {y_customer_labels_dl.shape}")
...
print(f"X_test_xgb shape: {X_test_xgb.shape}, y_test_xgb_monthly shape: {y_test_xgb_monthly.shape}")
print(f"X_train_dl_seq shape: {X_train_dl_seq.shape}, y_train_dl_cust shape: {y_train_dl_cust.shape}")
print(f"X_test_dl_seq shape: {X_test_dl_seq.shape}, y_test_dl_cust shape: {y_test_dl_cust.shape}")
print(f"Number of test customers: {len(test_cust_ids)}")
print(f"Actual tampering rate in test customers: {y_test_dl_cust.mean():.4f}")
...
print(f"Autoencoder anomaly threshold set at: {threshold_ae:.4f} (95th percentile of normal training errors)")
...
print("\nConfusion Matrix (Combined OR Logic):")
print(cm_combined)

TN_combined, FP_combined, FN_combined, TP_combined = cm_combined.ravel()
print(f"\nTrue Negatives (TN): {TN_combined}")
print(f"False Positives (FP): {FP_combined}")
print(f"False Negatives (FN): {FN_combined}")
print(f"True Positives (TP): {TP_combined}")

precision_combined = precision_score(y_test_combined, combined_predictions_or)
recall_combined = recall_score(y_test_combined, combined_predictions_or)
f1_combined = f1_score(y_test_combined, combined_predictions_or)

print(f"\nPrecision (Combined OR, Tampering): {precision_combined:.4f}")
print(f"Recall (Combined OR, Tampering): {recall_combined:.4f}")
print(f"F1-Score (Combined OR, Tampering): {f1_combined:.4f}")
print("\nClassification Report (Combined OR Logic):")
print(classification_report(y_test_combined, combined_predictions_or, target_names=['Normal (0)', 'Tampering (1)']))
...
precision_xgb_cust = precision_score(y_test_combined, y_pred_xgb_binary_customer_level)
recall_xgb_cust = recall_score(y_test_combined, y_pred_xgb_binary_customer_level)
f1_xgb_cust = f1_score(y_test_combined, y_pred_xgb_binary_customer_level)
print(f"\nXGBoost (Customer Level):")
print(f"  Precision: {precision_xgb_cust:.4f}, Recall: {recall_xgb_cust:.4f}, F1-Score: {f1_xgb_cust:.4f}")
print(f"  Confusion Matrix:\n{cm_xgb_cust}")

# Autoencoder Evaluation (customer-level)
cm_ae_cust = confusion_matrix(y_test_combined, y_pred_ae_binary_customer_level)
precision_ae_cust = precision_score(y_test_combined, y_pred_ae_binary_customer_level)
recall_ae_cust = recall_score(y_test_combined, y_pred_ae_binary_customer_level)
f1_ae_cust = f1_score(y_test_combined, y_pred_ae_binary_customer_level)
print(f"\nAutoencoder (Customer Level):")
print(f"  Precision: {precision_ae_cust:.4f}, Recall: {recall_ae_cust:.4f}, F1-Score: {f1_ae_cust:.4f}")
print(f"  Confusion Matrix:\n{cm_ae_cust}")


print("\n--- End of Meter Tampering Detection Framework ---")
print("This script now loads data from a CSV, demonstrating a more realistic workflow.")
print("The next step is to discuss the full conceptual framework for your PhD, covering deployment and continuous learning.")
